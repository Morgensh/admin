...
canvas.addEventListener('touchstart',e=>{
  ...
  if(pts.length===1){
    pinch.on=false;
    drag={on:true,sx:pts[0].x,sy:pts[0].y,vx0:vx,vy0:vy,moved:0,lx:pts[0].x,ly:pts[0].y};
  } else if(pts.length===2){
    // второй палец лёг на экран — переключаемся с пана на pinch-zoom, запоминаем стартовую дистанцию/масштаб
    drag.on=false;
    pinch.on=true;
    pinch.d0=Math.hypot(pts[0].x-pts[1].x,pts[0].y-pts[1].y)||1;
    pinch.vs0=vs;pinch.vx0=vx;pinch.vy0=vy;
    pinch.mx0=(pts[0].x+pts[1].x)/2;pinch.my0=(pts[0].y+pts[1].y)/2;
  }
},{passive:false});
canvas.addEventListener('touchmove',e=>{
  ...
  const pts=Object.values(touches);
  if(pts.length===2&&pinch.on){
    const rect=canvas.getBoundingClientRect();
    const d=Math.hypot(pts[0].x-pts[1].x,pts[0].y-pts[1].y)||1;
    const mx=(pts[0].x+pts[1].x)/2,my=(pts[0].y+pts[1].y)/2;
    const newVs=Math.max(.4,Math.min(14,pinch.vs0*(d/pinch.d0)));
    // точка мира, которая была под пальцами в момент начала жеста (по стартовому vs/vx/vy)
    const wx=(pinch.mx0-rect.left-pinch.vx0)/pinch.vs0;
    const wy=(pinch.my0-rect.top-pinch.vy0)/pinch.vs0;
    vs=newVs;
    // держим эту же точку мира под текущим центром пальцев — зум идёт от места жеста, а не от угла экрана
    vx=mx-rect.left-wx*vs;
    vy=my-rect.top-wy*vs;
    clamp();
  } else if(pts.length===1&&drag.on){
    drag.moved+=Math.hypot(pts[0].x-drag.lx,pts[0].y-drag.ly);
    drag.lx=pts[0].x;drag.ly=pts[0].y;
    vx=drag.vx0+(pts[0].x-drag.sx);vy=drag.vy0+(pts[0].y-drag.sy);
    clamp();
  }
},{passive:false});
canvas.addEventListener('touchend',e=>{
  e.preventDefault();
  const last=Object.values(touches);
  for(const t of e.changedTouches)delete touches[t.identifier];
  const remaining=Object.values(touches);
  if(pinch.on&&remaining.length<2)pinch.on=false;
  if(!remaining.length&&drag.on&&drag.moved<14)handleClick(last[0].x,last[0].y);
  if(!remaining.length)drag.on=false;
},{passive:false});
...
canvas.addEventListener('touchcancel',e=>{for(const t of e.changedTouches)delete touches[t.identifier];if(!Object.keys(touches).length){drag.on=false;pinch.on=false;}},{passive:false});
...
function resize(){
  // canvas.width/height задаём в физических пикселях (CSS-размер * devicePixelRatio) — без этого
  // на retina-экранах (dpr=2-3) canvas рендерится в заниженном разрешении и браузер сам его растягивает,
  // отсюда размытость. style.width/height держим в CSS-пикселях, чтобы элемент на странице не менял размер.
